...
def get_date_last_played(record):
    date_last_played = datetime.datetime.strptime(
        record['dateLastPlayed'], "%Y-%m-%dT%H:%M:%SZ")
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    add_devices(
        [Destiny2Sensor(name, membership_type, destiny_membership_id, api_key)])
...
class Destiny2Sensor(Entity):
    ...
    def update(self):
        ...
        """
        try:
            url = "https://www.bungie.net/Platform/Destiny2/" + self._membership_type + \
                "/Profile/" + self._destiny_membership_id + "/?components=200,205"
            ...
            sorted_chars_list = sorted(dict_to_list(
                characters), key=get_date_last_played, reverse=True)
